...
def to_markdown(report):
    ...
    if report.get("attack"):
        ...
        for tech in report["attack"]:
            ...
            lines.append(
                f"| {_md_cell(tech['tactic'])} | [{tech['id']}]({url}) "
                f"| {_md_cell(tech['name'])} | {_md_cell(', '.join(tech['evidence'][:3]))} |"
            )
        if report.get("attack_story"):
            lines += ["", report["attack_story"]]
...
def to_html(report):
    ...
    if report.get("attack"):
        ...
        for t in report["attack"]:
            ...
            arows.append([esc(t["tactic"]), _html_a(url, t["id"]), esc(t["name"]),
                          esc(", ".join(t["evidence"][:3]))])
        attack_html = (f'<h2 class="sec">MITRE ATT&amp;CK ({len(report["attack"])} techniques)</h2>'
                       + _html_table(["Tactic", "Technique", "Name", "Evidence"], arows))
        if report.get("attack_story"):
            attack_html += ('<div class="ai">' + _md_to_html(report["attack_story"]) + "</div>")
...
{attack_html}
{ai_html}
<footer>Generated by SilentPivot</footer>
</div></body></html>"""
...
def default_filename(report: dict, ext: str) -> str:
    target = _fs_safe(report.get("target"), "target")
    ip = _fs_safe(report.get("resolved_ip"), "")
    stamp = datetime.now().strftime("%Y%m%d_%H%M")
    # Only append the IP when it adds info — i.e. the target is a hostname that resolved
    # to a different IP. An IP target (target == ip) is shown once, not "1.2.3.4(1.2.3.4)".
    name = f"{target}({ip})" if (ip and ip != target) else target
